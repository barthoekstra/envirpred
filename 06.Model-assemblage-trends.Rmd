...
as.data.frame(varimp(mod_trends_rac)) %>% arrange(desc(reduction))
```
...
as.data.frame(varimp(mod_resids_rac)) %>% arrange(desc(reduction))
```

##### All predictor models

```{r varimp_all, results='hold', out.width='100%'}
plot(varimp(mod_all), main = "Varimp All pred mdl")
plot(varimp(mod_all_rac), main = "Varimp All pred RAC mdl")
as.data.frame(varimp(mod_all_rac)) %>% arrange(desc(reduction))
```
...
```{r remove_unnecessary_rac_models, include=FALSE}
rm(mod_all_rac)
rm(mod_resids_rac)
rm(mod_trends_rac)
```
...
```{r partial_effects_simple_original, out.width='100%', fig.width=10, fig.height=7}
par(mfrow = c(2, 3))
plot(mod_trends)
...
```{r partial_effects_simple_original_all, out.width='100%', fig.width=5, fig.height=5}
...
rm(mod_resids)
rm(mod_trends)
mod_all_rac <- readRDS("data/processed/models/mod_all_rac.RDS")
mod_resids_rac <- readRDS("data/processed/models/mod_resids_rac.RDS")
mod_trends_rac <- readRDS("data/processed/models/mod_trends_rac.RDS")
```
