...
mod_all <- readRDS("data/processed/models/mod_all_rac.RDS")
```
...
```
And finally for the all-encompassing (`all`) model.

```{r plot_mboot_ci_all, results='hide', fig.show='asis', out.width='100%', fig.width=10, fig.height=10}
vars_all <- as.data.frame(modelci_all$data) %>% colnames()

par(mfrow = c(4, 3))
lapply(vars_all, function(x) plot(modelci_all, which = x))
```
