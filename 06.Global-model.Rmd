...
mod_trends <- gamboost(formula_trends[[1]], data = data_subset, family = Gaussian(), control = ctrl)
mod_resids <- gamboost(formula_resids[[1]], data = data_subset, family = Gaussian(), control = ctrl)
...
data_subset %>%
  select(x, y, norac_trends, norac_resids) %>%
...
data_subset %>%
  select(x, y, rac_trends, rac_resids) %>%
