plot.mboost_grid <- function(x, which = NULL, newdata = NULL,
  ...
  for (w in which) {
    ...
    plot_helper <- function(xl, yl){
      ...
      if (ncol(data) == 2 && n_levelplot == 1) {
        if (is.null(newdata)){
          if (!is.null(gridsize)) {
            probs <- seq(from = 0, to = 1, length.out = gridsize)
            tmp <- expand.grid(quantile(data[, 1], probs = probs), quantile(data[, 2], probs = probs))
          } else {
        ...
        if (!userspec.ylab)
          yl <- colnames(data)[2]
        RET <<- lattice::levelplot(fm, data = data, xlab = xl, ylab = yl, ...)
      }
      if (ncol(data) > 2) {
